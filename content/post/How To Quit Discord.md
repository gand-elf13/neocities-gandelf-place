...
lastmod: 2026-05-21T19:21:10.076Z
---
2025-07-24Page récupérée de \[@JoinMatrixGuide]\[1]\
j'y ajoute mes choix comme je suis un nouvel utilisateur de matrix.\
j'ai crée mon compte sur federated.nexus qui est ouvert aux inscriptions\
j'utilise cinny sur mon ordinateur dans sa version web car il resemble bcp a discord\
sur mobile j'ai element X que je n'aime aps je cherche une alternative.
...
* **Freedom.** You get to choose how to use the platform and how your data is treated.
  * Conversations can be end-to-end encrypted[1](#fn:1). This is enabled by default for direct and group messages[2](#fn:2)
  * Automatic collection of data is minimized: no address book syncing[3](#fn:3), no “are you human?” checks beyond registration, phone numbers are optional, even email addresses are optional (on select homeservers)!
  * You can host your own server, or you can join [an existing public homeserver](https://joinmatrix.org/servers) that suits you. Either way, your access to the Matrix federation is the same[4](#fn:4)!
...
* **Trust.** You get to actually trust the software you use.
  * Matrix is an open protocol, and most[5](#fn:5) of its [apps](#what-app-should-i-use) and servers[6](#fn:6) are both open source, which you may contribute to!
...
Matrix is not spotless, but it is constantly improving! You can also follow [the Matrix.org blog](https://matrix.org/blog/posts) for all the news around Matrix!
...
The "default" Matrix homeserver is `matrix.org`, which is used by 35% of all Matrix users [as estimated in 2020](https://matrix.org/blog/2020/01/02/on-privacy-versus-freedom). Although it is okay to use it (and you can try out Matrix quickly with it), it is highly encouraged to choose a different homeserver (including running your own) for long-term usage, as it serves the spirit of decentralization promoted by the Matrix protocol, and also because `matrix.org` is occasionally overloaded (though performance has improved in recent times) and behind Cloudflare (which is a security risk).
...
When you log into a new device, you will be prompted to verify it using your existing device (by scanning a QR code or by comparing emojis). Your new device will then retrieve the room keys from your existing device, thereby enabling it to read your encrypted messages. This prevents anyone else - including your homeserver operator - to read encrypted content[1](#fn:1).

It is **strongly recommended** to check for new sessions. (Note that Element does notify you on new sessions.) If you do not recognize the session, then **you must remove it immediately!** Not doing so can [compromise your future encrypted messages](https://blog.erethon.com/blog/2022/07/13/what-a-malicious-matrix-homeserver-admin-can-do/#headline-4).
...
For PC users, if you want a better experience when using Element Desktop or SchildiChat, place [this config file](https://joinmatrix.org/assets/config.json) in your [configuration folder](https://github.com/vector-im/element-desktop#user-specified-configjson), where `$NAME` is either "Element", "Element Nightly" or "SchildiChat". The config file enables the "Labs" tab in settings, enables custom themes, preloads [a few custom themes](https://github.com/aaronraimist/element-themes), and preloads a few homeservers for room directory searches.
...
1. Only text contents and file attachments of messages are encrypted. Currently, Matrix does not prevent metadata leakage, mainly due to its federated nature. This will change, however, when Matrix starts rolling out [Pinecone](https://archive.fosdem.org/2021/schedule/event/matrix_pinecones/), allowing p2p connections. Currently, it *can* be mitigated if all participants of an E2EE conversation are running their own homeserver (so to eliminate third parties). [↩](#fnref:1) [↩2](#fnref:1:1)

2. Exception: Some bots do not support end-to-end encrypted messaging. Furthermore, when creating an empty private room, you will be prompted (but not by default) to enable encryption. [↩](#fnref:2)

3. Element allows you to opt into (not enabled by default as of late 2021) using an “identity server” - think of it as a big online address book. This allows users to share their email addresses and username, which can be looked up manually by other users. However, here “address book” means that Matrix will not store the one locally on your phone; homeservers *can* see who you are talking to, as such information are not encrypted. There is [a proposal](https://github.com/matrix-org/matrix-doc/pull/3414) to address this. See also footnote 1. [↩](#fnref:3)

4. Note that public rooms may block certain servers - just like banning individual users - due to prevalence of unacceptable content (spam, hate speech, etc.). If you’re not running your own homeserver, don’t join homeservers that are known to harbour such content. This does not apply to homeservers listed on [our public list](https://joinmatrix.org/servers) as they are vetted against any presence of bad reputation. In any case, behave yourselves, remember the human. [↩](#fnref:4)

5. This includes all clients and servers that an average user uses. [↩](#fnref:5)

6. [Synapse](https://github.com/matrix-org/synapse/) is the only stable homeserver implementation as of now. If you are living on the edge, you can try out [Dendrite](https://github.com/matrix-org/dendrite/) and [Conduit](https://conduit.rs/), both of which aim to support p2p eventually (see footnote 1). [↩](#fnref:6) [↩2](#fnref:6:1)

7. In most cases, it is equivalent to the server part of your MXID. The exceptions are where homeservers did not set up `.well-known` autodiscovery properly… [↩](#fnref:7)

8. Title and description. [↩](#fnref:8)

9. Which is not a reasonable gauge of activity. First, accounts can be inactive. Second, if a room uses [bridges](https://joinmatrix.org/guide/features/#all-about-bridges), then these accounts are counted as well, even if their activity is mostly not from Matrix (you can still interact with them, however). [↩](#fnref:9)

10. Which, to be exact, is a special type of rooms. Other than that you can list rooms (and Spaces) instead of sending messages, there are no differences between a Space and a room. [↩](#fnref:10)

### This Page is Open Source

[Help improve its content by opening a Pull Request on GitHub](https://github.com/austinhuang0131/joinmatrix/edit/main/guide.md).

\[1]

“The ‘Join Matrix!’ Guide,” Join Matrix! Accessed: Feb. 13, 2026. \[Online]. Available: <https://joinmatrix.org/guide/>
